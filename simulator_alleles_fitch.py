...
def load_simulation_results(output_file):
    ...
    with open(output_file, "r") as f:
        for lineno, line in enumerate(f, start=1):
            try:
                ...
                print(f"Fehler beim Parsen der JSON in Zeile {lineno}: {e}")
                continue
            
            try:
                hgt_rate = data["hgt_rate"]  # Damit wir hgt_rate kennen, falls Fehler weiter unten kommt
                
                hgt_rates.append(hgt_rate)
                rhos.append(data["rho"])
                fitch_scores_list.append(data["fitch_score"])
                gene_number_hgt_events_passed_list.append(data["gene_number_hgt_events_passed"])
                gene_presence_absence_matrices.append(np.array(data["matrix"]))
                ...
                alleles_list_pca_lists = data["alleles_list_pca"]

                alleles_list_pca_matrices = [reconstruct_distance_matrix(
                    np.array(alleles_list_pca_lists[i]), gene_absence_presence = gene_presence_absence_matrices[0][i]) for i in range(len(alleles_list_pca_lists))]
                alleles_list_pca_all_runs.append(alleles_list_pca_matrices)
           
            except Exception as e:
                print(f"Fehler bei Datenverarbeitung für hgt_rate={hgt_rate} in Zeile {lineno}: {e}")

    return hgt_rates, rhos, fitch_scores_list, gene_number_hgt_events_passed_list, gene_presence_absence_matrices, alleles_list_pca_all_runs, distance_matrices


def load_simulation_results_h5(output_file):
    hgt_rates = []
    rhos = []
    fitch_scores_list = []
    gene_number_hgt_events_passed_list = []
    ...
    with h5py.File(output_file, "r") as f:
        for run_id in f.keys():
            ...
            try:
                ...
                # Sortierung sicherstellen, falls durchnummeriert
                pca_keys.sort()
                for key in pca_keys:
                    alleles_pca_raw = grp[key][:]
                    # im Originalcode wird gene_presence_absence_matrices[0][i] verwendet
                    # hier nehmen wir analog matrix[i], weil das die gleiche Struktur liefert
                ...
                print(f"Fehler bei Datenverarbeitung für run_id={run_id}: {e}")
...
def simulate_and_store(theta, rho, num_samples, num_genes, hgt_rate, ce_from_nwk, distance_matrix_core, output_file):
    ...
    """
    if gene_absence_presence_matrix.sum() > 0: # Gene at least present once in the leafs
        output_dict = {
            "hgt_rate": hgt_rate,
            "rho": rho,
            "fitch_score": fitch_scores,
            "gene_number_hgt_events_passed": gene_number_hgt_events_passed,
            "matrix": gene_absence_presence_matrix.tolist(),
            "alleles_list_pca": [np.round(matrix, 3).tolist() for matrix in alleles_list_pca],
...
#@profile
def run_simulation(same_core_tree, num_simulations, output_file, theta, hgt_rate_samples, rho_samples, num_samples, num_genes):
    ...
    for start in range(0, num_simulations, chunk_size):
        ...
        args_list = [
            (theta, rho_samples[idx].item(), num_samples, num_genes, hgt_rate_samples[idx].item(), ce_from_nwk, distance_matrix_core, output_file)
    ...
    with ProcessPoolExecutor(max_workers=5) as executor:
        ...
        for idx in range(num_simulations):
            hgt_rate = hgt_rate_samples[idx].item()
            rho = rho_samples[idx].item()
            futures.append(executor.submit(simulate_and_store, theta, rho, num_samples, num_genes, hgt_rate, ce_from_nwk, output_file))
    ...
    """
    # hgt_rate = 0.5
    # simulate_and_store(theta, rho, num_samples, num_genes, hgt_rate, ce_from_nwk, output_file)
...
if __name__ == '__main__':
    ...
    output_file = "C:\\Users\\uhewm\\Desktop\\SBI_Data\\simulation_results.h5"

    if os.path.exists(output_file):
        os.remove(output_file)

    run_simulation(same_core_tree, num_simulations, output_file, theta, hgt_rate_samples, rho_samples, num_samples, num_genes)
